...
include_once("RotateDoge.php");
}
if ($BtnLtc == "STOP") {
date_default_timezone_set('UTC');
$atTMlite = $arrayTimeC['timebtnltc'];
$datebrLtc = date("d-m-Y H:i:s");
$tmOurlite = (strtotime($datebrLtc)- strtotime($atTMlite));  //-------cntdfirst ????
mysql_query("UPDATE ltcdogeming SET cntdtwo = '$tmOurlite' WHERE vlogin = '$login'  AND vpassword = '$password' AND activation='1' ");
include("scrlitejav.php");
print "<body onload=\"Liteup();\">";
include_once("RotateLtc.php");
}
if ($BtnDvs == "STOP") {
date_default_timezone_set('UTC');
...
$intspdDVSm = number_format (($MiniPower/3.2), 11, '.','');
...
$intspdHSP = number_format (($CntMXC/3.1), 11, '.','');
...
$BITDVSmon = number_format(($DVST * $PriceDVSONE)/2.9,11,'.','');
$CloudDVSb = number_format($DVST,11,'.','');
}
//-----------------------------------------------------------------------
$BITmini = (($MiniPower * $PriceDVSONE)/3.2);
...
$BITminimon = number_format(($MiniPower * $PriceDVSONE)/3.2,11,'.','');
$Cloudminibit = number_format($MiniPower,11,'.','');
}
//-----------------------------------------------------------------------
$BITsmc = (($CntMXC * $PriceDVSONE)/3.1);
...
$BITsmcmon = number_format(($CntMXC * $PriceDVSONE)/3.1,11,'.','');
...
$threedvsbit = ($yeardvsbit *3);
$daysmcbit = ($BITsmc / 30);
$yearsmcbit = ($BITsmc * 12);
$threesmcbit = ($yearsmcbit * 3);
$dayminibit = ($BITmini / 30);
$yearminibit = ($BITmini * 12);
$threeminibit = ($yearminibit * 3);
//-------------Doge-----hash500+hash300-------------------
//-------------hash500------------
$LBDDoNEWge = (($MiniPower * $PriceDogeNF)/3.2);
...
$OURDGmini = number_format(($MiniPower * $PriceDogeNF)/3.2,11,'.','');
$CloudFIVE = number_format($MiniPower,11,'.','');
}
//--------------end-hash500-----------
//--------------hash300---------------
$LBDDogNEWe = (($CntMXC * $PriceDogeNF)/3.1);   // ОБЩИЕ ПЕРЕМЕННЫЕ ПРАЙСОВ +++
...
$OURDGsmc = number_format(($CntMXC * $PriceDogeNF)/3.1,11,'.','');
...
$LBDdogthree = ($LBDdogyear * 3);
$LBDDogefiday = ($LBDDoNEWge / 30);
$LBDDogefiyear = ($LBDDoNEWge * 12);
$LBDDogefithree = ($LBDDogefiyear * 3);
//--------------------end-hash500+hash300------------------------------------
//-------------LITECOIN-----hash500+hash300-------------------
//-------------hash500------------
$LBDLiNEWte = (($MiniPower * $PriceLtcNF)/3.2);
...
$OURLTCmini = number_format(($MiniPower * $PriceLtcNF)/3.2,11,'.','');
$CloudFIVEL = number_format($MiniPower,11,'.','');
}
//--------------end-hash500-----------
//--------------hash300---------------
$LBDLitNEWe = (($CntMXC * $PriceLtcNF)/3.1);   // ОБЩИЕ ПЕРЕМЕННЫЕ ПРАЙСОВ +++
...
$OURLTCsmc = number_format(($CntMXC * $PriceLtcNF)/3.1,11,'.','');
...
$LBDlitethree = ($LBDliteyear * 3);
$LBDlitefiday = ($LBDLiNEWte / 30);
$LBDlitefiyear = ($LBDLiNEWte * 12);
$LBDlitefithree = ($LBDlitefiyear * 3);
//--------------------end-hash500+hash300------------------------------------
...
$intspeedDVS = number_format (($DVST/2.9), 11, '.','');
...
echo "<script>alert(\"Please buy power for the start of mining! ".""."\");</script>";
...
$Refpercentd = (($yzCountedCCBDogeM *4)/ 100);
...
$Refpercentl = (($yzCountedCCBLiteNR *4)/ 100);
...
$Refpercent = (($yzCountedCCBNEW *4)/ 100);
...
$Reherherl = mysql_fetch_array($DBherrefl) or die(mysql_error());
$Refmaincashl = $Reherherl['daycashlite'];
$Refpercentl = (($yzCountedCCBLiteNDO *4)/ 100);
$RefpercentlLOR = number_format($Refpercentl,8,'.','');
$Refoursuml = ($Refmaincashl + $RefpercentlLOR);
$RefoursumlLOR = number_format($Refoursuml,8,'.','');
mysql_query("UPDATE greffer SET daycashlite = '$RefoursumlLOR' WHERE my_refflink='$ReReflink' ");
...
$Reherherl = mysql_fetch_array($DBherrefl) or die(mysql_error());
$Refmaincashl = $Reherherl['daycashlite'];
$Refpercentl = (($yzCountedCCBLiteNEW *4)/ 100);
$RefpercentlLOR = number_format($Refpercentl,8,'.','');
$Refoursuml = ($Refmaincashl + $RefpercentlLOR);
$RefoursumlLOR = number_format($Refoursuml,8,'.','');
mysql_query("UPDATE greffer SET daycashlite = '$RefoursumlLOR' WHERE my_refflink='$ReReflink' ");
